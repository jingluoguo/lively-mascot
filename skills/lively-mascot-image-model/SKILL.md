...
When the reference has only one facial vocabulary, such as two simple eyes with no mouth, eyebrows, or pupils, use that existing feature as the expression channel. For non-neutral eye actions, add a clearly visible but restrained local change such as a 10-25% scale increase, spacing change, tilt, or squash. Do not invent a mouth or unrelated facial anatomy merely to make an emotion readable.
...
For 3D-capable models, preserve the native layered body, face, and upper/lower feature structure. The same locked-gaze neutral frame must visibly differ between 2D and 3D through artwork layers, shadows, and depth, not a rectangular wrapper or a rotated full model. The complete body remains upright in all 40 states; local ears, tail, leaf, antenna, or accessories may animate independently. Never apply `rotate()`, `skew()`, or a directional `translateX()` to the complete body, its primary artwork layer, `.lively-mascot__rig`, or `.lively-mascot__gaze`. When core state animations would rotate an upright model, replace them with model-scoped vertical, scale, opacity, or color motion and verify every state with `followCursor:false`.
