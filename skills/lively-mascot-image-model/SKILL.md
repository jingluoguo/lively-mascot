...
Part actions are atomic, composable presentation primitives. Reuse `LivelyMascot.partActions.<part>` when declaring supported actions; do not invent a parallel action vocabulary or bind behavior to numeric emotion IDs. A custom emotion can combine one action per declared part through `recipe.parts`, and model CSS should consume `data-mascot-action-<part>` while making alternatives on the same part mutually exclusive.
...
### Face Variant Regeneration Contract

`setFaceVariant()` changes the root to exactly one of `.lively-mascot--face-default`, `.lively-mascot--face-simple`, or `.lively-mascot--face-dot`. The shared face already supports those classes. Every custom-face model with visible eyes must support all three as well: create a `__eye-default` primitive that preserves the measured reference geometry, a reduced `__eye-simple` capsule or line, and a `__eye-dot` circle in every registered eye group. Model CSS must hide all three by default and reveal exactly the selected primitive using those root classes; use `display: none` for any unselected replacement artwork that an action selector could otherwise reveal.

Face variants are a visual override, not separate expression states. Eye actions and blinks must pose the registered eye wrapper through CSS variables or transforms while retaining the selected primitive; they must not reveal a heart, closed-eye path, pupil, or default eye on top of the simple or dot face. A generated `model.json` must record `faceVariants: ["default", "simple", "dot"]`. During QA, switch through all three variants after neutral, closed, happy, wide, thinking, and love eye actions, and confirm that every eye group has exactly one visible eye primitive.

### Gaze Regeneration Contract

Every regenerated custom-eye model must use all six engine variables in the rigged eye-node transform, in this order: `translate(var(--lively-gaze-x), var(--lively-gaze-y))`, `rotate(var(--lively-gaze-rotate))`, `scaleX(var(--lively-gaze-depth))`, `scaleY(var(--lively-gaze-height))`, and `scale(var(--lively-gaze-scale))`. Register `verticalScale` with the eye gaze spec. Translation is mandatory: do not turn gaze into squash-only motion. For separate left/right nodes, declare `side`, `sideScale`, and `verticalScale`; the near eye should narrow modestly and the far eye should open, while looking up/down changes both eyes' vertical opening. For sparse, high-contrast two-eye faces with safe empty facial space, set `maxX` to 55-70% of visible eye width and `maxY` to 25-35% of its height; use the 35-55% horizontal range only when the silhouette leaves less room. Verify each cursor extreme has visible positional travel plus distinct horizontal, vertical, and near/far shape cues at the demo's rendered size.
...
For 3D-capable models, preserve the native layered body, face, and upper/lower feature structure. The neutral `02` frame must visibly differ between 2D and 3D through artwork layers, shadows, and depth, not a rectangular wrapper or an unintended lean. For a custom SVG/canvas model using `.lively-body--custom-model`, explicitly neutralize the higher-specificity shared 3D body `box-shadow`, `filter`, and pseudo layers with a model-scoped selector such as `.lively-mascot.lively-mascot--3d .<slug>__body`; put depth treatment on the actual artwork rather than its rectangular HTML wrapper. If an SVG owns its outline, also provide a `.lively-mascot--outline-hidden` selector that removes its stroke or outline layer, because the generic body border toggle cannot do that automatically. Neutral `02` keeps the complete body centered and upright with no residual rotation, skew, directional horizontal shift, visible rectangular plane, or stale outline. Non-neutral states may use controlled body or rig rotation, lateral movement, and scale when these clearly express the state and return cleanly to the neutral frame. An eyes-only gaze model may move its registered eye groups through the gaze variables above without moving the neutral body. Verify both the neutral reset and intentional non-neutral motion with `followCursor:false`.
...
- `model.json` with the declared parts, skin slots, effect anchors, face variants, the semantic behavior tags used by the model, Rig capability overrides, and actual limitations. Do not treat numeric emotion IDs as model capabilities.
